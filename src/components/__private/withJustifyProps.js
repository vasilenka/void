...
export const JustifyPropTypes = {
  justify: (props, propName, componentName) => {
    ...
    if (props.justify && !includes(justifys, props.justify)) {
      return new Error(`Invalid prop size='${props.justify}' supplied to ${componentName}`)
    }

    if (props.justify && some(justifys, justify => has(props, justify))) {
      return new Error(
        `Seems that you've accidentially supplied boolean size along with size='${props.justify}' to ${componentName}, please remove one of them. Otherwise boolean prop will overwrite the 'size' prop.`
      )
    }
  },
