...
export const SpacePropTypes = {
  space: (props, propName, componentName) => {
    if (props.space && !includes(spaces, props.space)) {
      return new Error(`Invalid prop space='${props.space}' supplied to ${componentName}`)
    }

    if (props.space && some(spaces, space => has(props, space))) {
      return new Error(
        `Seems that you've accidentially supplied boolean space along with space='${props.space}' to ${componentName}, please remove one of them. Otherwise boolean prop will overwrite the 'space' prop.`
      )
    }
  },
