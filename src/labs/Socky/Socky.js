...
const Socky = ({
  ...
  const handleZoom = () => {
    let left = (sockyRef.current.getContainerData().width - sockyRef.current.getCropBoxData().width) / 2
    let top = (sockyRef.current.getContainerData().height - sockyRef.current.getCropBoxData().height) / 2
  ...
  return (
    <div
      ...
      {...restProps}>
      <SockyToolbar>
        <Text className={styles.title} heading2 style={{ color: '#ffffff' }} component="h2">
          Edit photo
        </Text>
        <div>
          <SockyZoom
        ...
        <footer className={styles.footer}>
          ...
          <Button secondaryAlt light className={styles.cancel} onClick={handleClose}>
            Cancel
          </Button>
        </footer>
      </SockyToolbar>
